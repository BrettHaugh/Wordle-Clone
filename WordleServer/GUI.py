...
class WordleClientGUI:
    def __init__(self, root):
        ...
        self.server_ip = '10.30.112.56'  # Replace with your server's IP address
        ...
        guess_entry = tk.Entry(root, textvariable=self.guess_var, font=('Helvetica', 18), bd=3)
        guess_entry.bind('<Return>', self.make_guess)
        guess_entry.pack(pady=20)

    def create_tile(self, row, col):
        frame = tk.Frame(self.root, width=60, height=60, bg='lightgrey', highlightbackground='black', highlightthickness=2)
        frame.grid(row=row, column=col, padx=5, pady=5)
        frame.pack_propagate(False)
        label = tk.Label(frame, text="", bg='lightgrey', fg='white', font=('Helvetica', 18, 'bold'))
